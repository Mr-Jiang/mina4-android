...
/**
 * ProxyFilter.java - Proxy {@link IoFilter}. Automatically inserted into the
 * {@link IoFilter} chain by {@link ProxyConnector}. Sends the initial handshake
 * message to the proxy and handles any response to the handshake. Once the
 * handshake has completed and the proxied connection has been established this
 * filter becomes transparent to data flowing through the connection.
 * <p>
...
public class ProxyFilter extends IoFilterAdapter {
	...
	private static final String TAG = ProxyFilter.class.getName();
	...
	/**
	 * Called before the filter is added into the filter chain. Checks if chain
	 * already holds an {@link ProxyFilter} instance.
	 * 
	 * @param chain
	 *            the filter chain
	 * @param name
	 ...
	 *             if chain already contains an instance of {@link ProxyFilter}
	 */
	@Override
	...
	/**
	 * Called when the filter is removed from the filter chain. Cleans the
	 * {@link ProxyIoSession} instance from the session.
	 * 
	 * @param chain
	 *            the filter chain
	 * @param name
	...
	/**
	 * Receives data from the remote host, passes to the handler if a handshake
	 * is in progress, otherwise passes on transparently.
	 * 
	 * @param nextFilter
	 ...
	 *            the session object
	 * @param message
	 *            the object holding the received data
	 */
	@Override
	public void messageReceived(final NextFilter nextFilter, final IoSession session, final Object message)
		...
		synchronized (handler) {
			...
			if (handler.isHandshakeComplete()) {
				...
				Mina4Log.d(TAG, "Data Read: " + handler + " (" + buf + ")");
				...
				while (buf.hasRemaining() && !handler.isHandshakeComplete()) {
					Mina4Log.d(TAG, "Pre-handshake - passing to handler");
				...
				if (buf.hasRemaining()) {
					Mina4Log.d(TAG, "Passing remaining data to next filter");
	...
	/**
	 * Actually write data. Queues the data up unless it relates to the
	 * handshake or the handshake is done.
	 * 
	 * @param nextFilter
	 *            the next filter in filter chain
	 * @param session
	...
	public void writeData(final NextFilter nextFilter, final IoSession session, final WriteRequest writeRequest,
		...
		synchronized (handler) {
			if (handler.isHandshakeComplete()) {
				...
				Mina4Log.d(TAG, "handshake data: " + writeRequest.getMessage());
				...
				if (!session.isConnected()) {
					...
					Mina4Log.d(TAG, "Write request on closed session. Request ignored.");
				} else {
					// Queue the data to be sent as soon as the handshake
					// completes
					Mina4Log.d(TAG, "Handshaking is not complete yet. Buffering write request.");
	...
	/**
	 * Filter handshake related messages from reaching the messageSent callbacks
	...
	/**
	 * Called when the session is created. Will create the handler able to
	 * handle the {@link ProxyIoSession#getRequest()} request stored in the
	 * session. Event is stored in an {@link IoSessionEventQueue} for later
	 * delivery to the next filter in the chain when the handshake would have
	 * succeed. This will prevent the rest of the filter chain from being
	 * affected by this filter internals.
	 * 
	 * Please note that this event can occur multiple times because of some http
	 * proxies not handling keep-alive connections thus needing multiple
	 * sessions during the handshake.
	 * 
	 * @param nextFilter
	 *            the next filter in filter chain
	 * @param session
	...
	public void sessionCreated(NextFilter nextFilter, IoSession session) throws Exception {
		Mina4Log.d(TAG, "Session created: " + session);
		ProxyIoSession proxyIoSession = (ProxyIoSession) session.getAttribute(ProxyIoSession.PROXY_SESSION);
		Mina4Log.d(TAG, "get proxyIoSession: " + proxyIoSession);
	...
	/**
	 * Event is stored in an {@link IoSessionEventQueue} for later delivery to
	 * the next filter in the chain when the handshake would have succeed. This
	 * will prevent the rest of the filter chain from being affected by this
	 * filter internals.
	 * 
	 * @param nextFilter
	 *            the next filter in filter chain
