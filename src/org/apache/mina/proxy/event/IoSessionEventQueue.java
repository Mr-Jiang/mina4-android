...
/**
 * IoSessionEventQueue.java - Queue that contains filtered session events while
...
public class IoSessionEventQueue {
	...
	private static final String TAG = IoSessionEventQueue.class.getName();
	...
	/**
	 * Queue of session events which occurred before the proxy handshake had
	 * completed.
	 */
	...
	private void discardSessionQueueEvents() {
		synchronized (sessionEventsQueue) {
			...
			Mina4Log.d(TAG, "Event queue CLEARED");
		}
	}

	/**
	 * Event is enqueued only if necessary : - socks proxies do not need the
	 * reconnection feature so events are always forwarded for these. - http
	 * proxies events will be enqueued while handshake has not been completed or
	 * until connection was closed. If connection was prematurely closed
	...
	public void enqueueEventIfNecessary(final IoSessionEvent evt) {
		...
		if (proxyIoSession.getHandler().isHandshakeComplete()) {
			...
			if (evt.getType() == IoSessionEventType.CLOSED) {
				...
				// Enqueue event cause it will not reach IoHandler but deliver
				// it to enable
				// session creation.
	...
	public void flushPendingSessionEvents() throws Exception {
		synchronized (sessionEventsQueue) {
			...
			while ((evt = sessionEventsQueue.poll()) != null) {
				Mina4Log.d(TAG, "Flushing buffered event: " + evt);
				evt.deliverEvent();
			}
		}
	}
	 ...
	 * 
	 * @param evt
	 *            the session event to enqueue
	 */
	private void enqueueSessionEvent(final IoSessionEvent evt) {
		synchronized (sessionEventsQueue) {
			Mina4Log.d(TAG, "Enqueuing event: " + evt);
