...
public class IoSessionEvent {
	...
	private static final String TAG = IoSessionEvent.class.getName();
	...
	/**
	 * The idle status if type value is {@link IoSessionEventType#IDLE}, null
	 * otherwise.
	 */
	...
	public void deliverEvent() {
		Mina4Log.d(TAG, "Delivering event " + this);
		deliverEvent(this.nextFilter, this.session, this.type, this.status);
	}

	/**
	 * Static method which effectively delivers the specified event to the next
	 * filter <code>nextFilter</code> on the <code>session</code>.
	 * 
	 * @param nextFilter
	 *            the next filter
	 * @param session
	 *            the session on which the event occured
	 * @param type
	 *            the event type
	 * @param status
	 *            the idle status should only be non null only if the event type
	 *            is {@link IoSessionEventType#IDLE}
	 */
