...
/**
 * HttpNTLMAuthLogicHandler.java - HTTP NTLM authentication mechanism logic
 * handler.
 * 
...
public class HttpNTLMAuthLogicHandler extends AbstractAuthLogicHandler {
	...
	private static final String TAG = HttpNTLMAuthLogicHandler.class.getName();
	...
	public void doHandshake(NextFilter nextFilter) throws ProxyAuthException {
		...
		if (step > 0) {
			Mina4Log.d(TAG, "sending NTLM challenge response");
			...
			Mina4Log.d(TAG, "sending NTLM negotiation packet");
		...
		if (step == 1) {
			...
			try {
				challengePacket = Base64
						.decodeBase64(challengeResponse.substring(5).getBytes(proxyIoSession.getCharsetName()));
			} catch (IOException e) {
