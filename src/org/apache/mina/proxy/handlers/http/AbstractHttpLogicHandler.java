 ...
 * {@link AbstractProxyLogicHandler} implementations. Provides HTTP request
 * encoding/response decoding functionality.
 * 
...
public abstract class AbstractHttpLogicHandler extends AbstractProxyLogicHandler {
	...
	private static final String TAG = AbstractHttpLogicHandler.class.getName();
	...
	/**
	 * A flag that indicates that this is a HTTP/1.1 response with chunked
	 * data.and that some chunks are missing.
	 */
	private boolean hasChunkedData;

	/**
	 * A flag that indicates that some chunks of data are missing to complete
	 * the HTTP/1.1 response.
	 */
	private boolean waitingChunkedData;

	/**
	 * A flag that indicates that chunked data has been read and that we're now
	 * reading the footers.
	 */
	 ...
	 * <code>responseData</code> {@link IoBuffer}.
	 */
	...
	/**
	 * Handles incoming data during the handshake process. Should consume only
	 * the handshake data from the buffer, leaving any extra data in place.
	 * 
	 * @param nextFilter
	 *            the next filter
	 * @param buf
	 *            the buffer holding received data
	 */
	@Override
	public synchronized void messageReceived(final NextFilter nextFilter, final IoBuffer buf)
			throws ProxyAuthException {
		Mina4Log.d(TAG, "messageReceived()");
		...
		try {
			if (parsedResponse == null) {
				...
				Mina4Log.d(TAG,
						"response header received: " + responseHeader.replace("\r", "\\r").replace("\n", "\\n\n"));
			...
			if (!hasChunkedData) {
				...
				if ("chunked".equalsIgnoreCase(
						StringUtilities.getSingleValuedHeader(parsedResponse.getHeaders(), "Transfer-Encoding"))) {
					// Handle Transfer-Encoding: Chunked
					Mina4Log.d(TAG, "Retrieving additional http response chunks");
					hasChunkedData = true;
			...
			if (hasChunkedData) {
				...
				while (waitingChunkedData) {
					if (contentLength == 0) {
						...
						if (contentLength > 0) {
							contentLength += 2; // also read chunk's trailing
												// CRLF
							decoder.setContentLength(contentLength, true);
						}
					}
			...
			Mina4Log.d(TAG, "end of response received: "
	...
	private void writeRequest0(final NextFilter nextFilter, final HttpProxyRequest request) {
		try {
			...
			Mina4Log.d(TAG, "write: " + data.replace("\r", "\\r").replace("\n", "\\n\n"));
	...
	/**
	 * Method to reconnect to the proxy when it decides not to maintain the
	...
	private void reconnect(final NextFilter nextFilter, final HttpProxyRequest request) {
		Mina4Log.d(TAG, "Reconnecting to proxy ...");
		...
		proxyIoSession.getConnector().connect(new IoSessionInitializer<ConnectFuture>() {
			...
			public void initializeSession(final IoSession session, ConnectFuture future) {
				Mina4Log.d(TAG, "Initializing new session: " + session);
				session.setAttribute(ProxyIoSession.PROXY_SESSION, proxyIoSession);
				proxyIoSession.setSession(session);
				Mina4Log.d(TAG, "setting up proxyIoSession: " + proxyIoSession);
