...
public class HttpProxyRequest extends ProxyRequest {
	...
	private static final String TAG = HttpProxyRequest.class.getName();
	...
	/**
	 * Constructor which creates a HTTP/1.0 GET request to the specified http
	 * URI.
	 * 
	 * @param httpURI
	 *            the target URI
	...
	/**
	 * Constructor which creates a GET request to the specified http URI using
	...
	/**
	 * Constructor which creates a request using the provided HTTP verb targeted
	 * at the specified http URI using the provided protocol version and setting
		...
		if (host == null) {
			...
			if (host == null && httpURI != null) {
				try {
					...
					Mina4Log.d(TAG, "Malformed URL: " + e.toString());
				}
			}
		}
	 ...
	 * 
	 * @param properties
	 *            The properties to add to the reqyest
	 */
	 ...
	 * 
	 * @param propNames
	 *            The list of property name to check
