...
/**
 * HttpDigestAuthLogicHandler.java - HTTP Digest authentication mechanism logic
 * handler.
 * 
...
public class HttpDigestAuthLogicHandler extends AbstractAuthLogicHandler {
	...
	private static final String TAG = HttpDigestAuthLogicHandler.class.getName();
	...
	public void doHandshake(NextFilter nextFilter) throws ProxyAuthException {
		...
		if (step > 0) {
			Mina4Log.d(TAG, "sending DIGEST challenge response");
			...
			try {
				map.put("response", DigestUtilities.computeResponseValue(proxyIoSession.getSession(), map,
						req.getHttpVerb().toUpperCase(), req.getProperties().get(HttpProxyConstants.PWD_PROPERTY),
	...
	public void handleResponse(final HttpProxyResponse response) throws ProxyAuthException {
		...
		if (step == 0) {
			...
			try {
				directives = StringUtilities
						.parseDirectives(challengeResponse.substring(7).getBytes(proxyIoSession.getCharsetName()));
			} catch (Exception e) {
