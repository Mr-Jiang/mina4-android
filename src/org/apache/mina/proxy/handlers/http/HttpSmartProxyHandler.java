 ...
 * forwarding a request to the appropriate authentication mechanism logic
 * handler.
 * 
...
public class HttpSmartProxyHandler extends AbstractHttpLogicHandler {
	...
	private static final String TAG = HttpSmartProxyHandler.class.getName();
	...
	public void doHandshake(final NextFilter nextFilter) throws ProxyAuthException {
		...
		if (authHandler != null) {
			...
			Mina4Log.d(TAG, "sending HTTP request");
	 ...
	 * <code>preferedOrder</code> is set then algorithms are selected from the
	 * list order otherwise the algorithm tries to select the most secured
	 * algorithm available first.
	 * 
	 * @param response
	 *            the proxy response
	 */
		...
		if (values == null || values.isEmpty()) {
			...
			if (method != -1) {
				try {
					...
					Mina4Log.d(TAG, "Following exception occured:" + ex.toString());
				}
			}
