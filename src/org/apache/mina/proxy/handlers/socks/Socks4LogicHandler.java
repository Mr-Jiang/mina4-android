...
/**
 * Socks4LogicHandler.java - SOCKS4/SOCKS4a authentication mechanisms logic
 * handler.
 * 
...
public class Socks4LogicHandler extends AbstractSocksLogicHandler {
	...
	private static final String TAG = Socks4LogicHandler.class.getName();
	...
	/**
	 * Encode a SOCKS4/SOCKS4a request and writes it to the next filter so it
	 * can be sent to the proxy server.
	 * 
	 * @param nextFilter
	 *            the next filter
	 * @param request
	...
	/**
	 * Handle a SOCKS4/SOCKS4a response from the proxy server. Test the response
	 * buffer reply code and call {@link #setHandshakeComplete()} if access is
	 * granted.
	 * 
	 * @param buf
	 *            the buffer holding the server response data.
	 * @throws Exception
	 *             if server response is malformed or if request is rejected by
	 *             the proxy server.
	 */
