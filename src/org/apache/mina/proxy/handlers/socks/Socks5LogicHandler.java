...
public class Socks5LogicHandler extends AbstractSocksLogicHandler {
	...
	private static final String TAG = Socks5LogicHandler.class.getName();
	 ...
	 *             if request's hostname charset can't be converted to ASCII.
	 */
	 ...
	 * @param request
	 *            the socks proxy request data
	 * @return the encoded buffer, if null then authentication step is over and
	 *         handshake process can jump immediately to the next step without
	 *         waiting for a server reply.
	 * @throws UnsupportedEncodingException
	 *             if some string charset convertion fails
	 * @throws GSSException
	 *             when something fails while using GSSAPI
	 */
	private IoBuffer encodeAuthenticationPacket(final SocksProxyRequest request)
		...
		case SocksProxyConstants.NO_AUTH:
			// In this case authentication is immediately considered as
			// successfull
	...
	private IoBuffer encodeGSSAPIAuthenticationPacket(final SocksProxyRequest request) throws GSSException {
		...
		if (ctx == null) {
			...
			Mina4Log.d(TAG, "Available mechs:");
			for (Oid o : manager.getMechs()) {
				if (o.equals(krb5OID)) {
					Mina4Log.d(TAG, "Found Kerberos V OID available");
				}
				Mina4Log.d(TAG, manager.getNamesForMech(o) + " with oid = " + o);
			}
		...
		if (token != null) {
			Mina4Log.d(TAG, "Received Token[" + token.length + "] = " + ByteUtilities.asHex(token));
		}
		IoBuffer buf = null;
		...
		if (!ctx.isEstablished()) {
			...
			// initSecContext
			if (token != null) {
				Mina4Log.d(TAG, "Sending Token[" + token.length + "] = " + ByteUtilities.asHex(token));
	...
	/**
	 * Encodes a SOCKS5 request and writes it to the next filter so it can be
	 ...
	 *            the request to send.
	 * @param step
	 *            the current step in the handshake process
	 */
	private void writeRequest(final NextFilter nextFilter, final SocksProxyRequest request, int step) {
		try {
			...
			if (step == SocksProxyConstants.SOCKS5_GREETING_STEP) {
				...
				// This step can happen multiple times like in GSSAPI auth for
				// instance
	...
	protected void handleResponse(final NextFilter nextFilter, final IoBuffer buf, int step) throws Exception {
		...
		if (step == SocksProxyConstants.SOCKS5_GREETING_STEP) {
			...
			if (buf.remaining() >= len) {
				...
				Mina4Log.d(TAG, "response status:  " + SocksProxyConstants.getReplyCodeAsString(status));
