...
public abstract class AbstractPollingIoProcessor<S extends AbstractIoSession> implements IoProcessor<S> {
	...
	private void read(S session) {
		...
		try {
			...
			if ((e instanceof IOException) && (!(e instanceof PortUnreachableException)
				...
				try {
					...
					if (!wakeupCalled.getAndSet(false) && (selected == 0) && (delta < 100)) {
						...
						if (isBrokenConnection()) {
							Mina4Log.w(TAG, "Broken connection");
						} else {
							...
							if (nbTries == 0) {
								Mina4Log.w(TAG, "Create a new selector. Selected is 0, delta = " + delta);
								registerNewSelector();
								nbTries = 10;
							} else {
								nbTries--;
