...
/**
 * The default implementation of {@link IoFilterChainBuilder} which is useful in
 * most cases. {@link DefaultIoFilterChainBuilder} has an identical interface
 * with {@link IoFilter}; it contains a list of {@link IoFilter}s that you can
 * modify. The {@link IoFilter}s which are added to this builder will be
 * appended to the {@link IoFilterChain} when
 * {@link #buildFilterChain(IoFilterChain)} is invoked.
 * <p>
 ...
 * existing {@link IoSession}s won't get affected by the changes in this
 * builder. {@link IoFilterChainBuilder}s affect only newly created
 * {@link IoSession}s.
 *
 * <pre>
 * IoAcceptor acceptor = ...;
...
public class DefaultIoFilterChainBuilder implements IoFilterChainBuilder {
	...
	private static final String TAG = DefaultIoFilterChainBuilder.class.getName();
	 ...
	 * 
	 * @param filterType
	 *            The FilterType we are looking for
	 * @return The found Entry
	 */
	 ...
	 * 
	 * @param name
	 *            The Filter's name we want to check if it's in the chain
	 ...
	 * 
	 * @param filter
	 *            The Filter we want to check if it's in the chain
	 ...
	 * 
	 * @param filterType
	 *            The FilterType we want to check if it's in the chain
	 ...
	 * 
	 * @param name
	 *            The Filter's name to remove from the list of Filters
	 ...
	 * 
	 * @param filter
	 *            The Filter we want to remove from the list of Filters
	 * @return The removed IoFilter
	 */
	 ...
	 * 
	 * @param filterType
	 *            The FilterType we want to remove from the list of Filters
	 * @return The removed IoFilter
	 */
	 ...
	 * 
	 * @param name
	 *            The name of the filter to replace
	 * @param newFilter
	...
	/**
	 * Replace a filter by a new one. We are looking for a filter type, but if
	 * we have more than one with the same type, only the first found one will
	 * be replaced
	 * 
	 * @param oldFilterType
	 *            The filter type to replace
	 * @param newFilter
	 *            The new filter to use
	 */
	...
	/**
	 * Clears the current list of filters and adds the specified filter mapping
	 * to this builder. Please note that you must specify a {@link Map}
	 * implementation that iterates the filter mapping in the order of insertion
	 * such as {@link LinkedHashMap}. Otherwise, it will throw an
	...
	private boolean isOrderedMap(Map<String, ? extends IoFilter> map) {
		...
		if (LinkedHashMap.class.isAssignableFrom(mapType)) {
			Mina4Log.d(TAG, mapType.getSimpleName() + " is an ordered map.");
			return true;
		}

		Mina4Log.d(TAG, mapType.getName() + "  is not a " + LinkedHashMap.class.getSimpleName());
		...
		while (type != null) {
			for (Class<?> i : type.getInterfaces()) {
				if (i.getName().endsWith("OrderedMap")) {
					Mina4Log.d(TAG, mapType.getSimpleName()
		...
		Mina4Log.d(TAG, mapType.getName() + " doesn't implement OrderedMap interface.");
		...
		try {
			...
			String msg = String.format("Failed to create a new map instance of %s.\n", mapType.getName());
			Mina4Log.d(TAG, msg + e.toString());
			return false;
		}
		...
		for (int i = 0; i < 65536; i++) {
			...
			for (Object key : newMap.keySet()) {
				if (!it.next().equals(key)) {
					String msg = String.format("The specified map didn't pass the insertion order test after %d tries.",
							(i + 1));
					Mina4Log.d(TAG, msg);
					return false;
				}
		...
		Mina4Log.d(TAG, "The specified map passed the insertion order test.");
		return true;
	}
