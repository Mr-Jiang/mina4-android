...
/**
 * A default implementation of {@link IoFuture} associated with an
 * {@link IoSession}.
 * 
...
public class DefaultIoFuture implements IoFuture {
	...
	/**
	 * The first listener. This is easier to have this variable when we most of
	 * the time have one single listener
	 */
	 ...
	 *
	 * @param session
	 *            an {@link IoSession} which is associated with this future
	 */
	 ...
	 * already an instance of the caller.
	 */
	 ...
	 * 
	 * @param newValue
	 *            The result to store into the Future
	 * @return {@code true} if the value has been set, {@code false} if the
	 *         future already has a value (thus is in ready state)
	 */
	public boolean setValue(Object newValue) {
		synchronized (lock) {
			...
			// Now, if we have waiters, notify them that the operation has
			// completed
			if (waiters > 0) {
				lock.notifyAll();
			}
