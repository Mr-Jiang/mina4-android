...
/**
 * An {@link IoProcessor} pool that distributes {@link IoSession}s into one or
 * more {@link IoProcessor}s. Most current transport implementations use this
 * pool internally to perform better in a multi-core environment, and therefore,
 * you won't need to use this pool directly unless you are running multiple
...
public class SimpleIoProcessorPool<S extends AbstractIoSession> implements IoProcessor<S> {
	...
	private static final String TAG = SimpleIoProcessorPool.class.getName();
	...
	/**
	 * Creates a new instance of SimpleIoProcessorPool with a default size of
	 * NbCPUs +1.
	 *
	 * @param processorType
	...
	/**
	 * Creates a new instance of SimpleIoProcessorPool with a defined number of
	 * IoProcessors in the pool
	 *
	 * @param processorType
	 *            The type of IoProcessor to use
	 * @param size
	...
	public SimpleIoProcessorPool(Class<? extends IoProcessor<S>> processorType, Executor executor, int size,
		...
		try {
			...
			try {
				...
				Mina4Log.e(TAG, "Cannot create an IoProcessor : " + re.getMessage());
				throw re;
			} catch (Exception e) {
				String msg = "Failed to create a new instance of " + processorType.getName() + ":" + e.getMessage()
						+ "\n";
				Mina4Log.e(TAG, msg + e.toString());
			...
			if (!disposing) {
				...
				for (IoProcessor<S> ioProcessor : pool) {
					...
					try {
						...
						String msg = String.format("Failed to dispose the %s IoProcessor.\n",
								ioProcessor.getClass().getSimpleName());
						Mina4Log.w(TAG, msg + e.toString());
					}
				}
