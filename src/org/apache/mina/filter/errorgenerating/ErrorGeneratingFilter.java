...
/**
 * An {@link IoFilter} implementation generating random bytes and PDU
 * modification in your communication streams. It's quite simple to use :
 * <code>ErrorGeneratingFilter egf = new ErrorGeneratingFilter();</code> For
 * activate the change of some bytes in your {@link IoBuffer}, for a probability
 * of 200 out of 1000 {@link IoBuffer} processed :
 * <code>egf.setChangeByteProbability(200);</code> For activate the insertion of
 * some bytes in your {@link IoBuffer}, for a probability of 200 out of 1000 :
 * <code>egf.setInsertByteProbability(200);</code> And for the removing of some
 * bytes : <code>egf.setRemoveByteProbability(200);</code> You can activate the
 * error generation for write or read with the following methods :
...
public class ErrorGeneratingFilter extends IoFilterAdapter {
	...
	private static final String TAG = ErrorGeneratingFilter.class.getName();
	...
	private IoBuffer insertBytesToNewIoBuffer(IoSession session, IoBuffer buffer) {
		if (insertByteProbability > rng.nextInt(1000)) {
			Mina4Log.i(TAG, buffer.getHexDump());
			...
			Mina4Log.i(TAG, "Inserted " + count + " bytes.");
			Mina4Log.i(TAG, newBuff.getHexDump());
			return newBuff;
		}
		return null;
	}
	...
	private void manipulateIoBuffer(IoSession session, IoBuffer buffer) {
		if ((buffer.remaining() > 0) && (removeByteProbability > rng.nextInt(1000))) {
			...
			buffer.put(newBuff);
			buffer.flip();
			Mina4Log.i(TAG, "Removed " + count + " bytes at position " + pos + ".");
			Mina4Log.i(TAG, buffer.getHexDump());
		}
		if ((buffer.remaining() > 0) && (changeByteProbability > rng.nextInt(1000))) {
			...
			Mina4Log.i(TAG, "Modified " + count + " bytes.");
			Mina4Log.i(TAG, buffer.getHexDump());
		}
	}
	...
	/**
	 * Set the probability for the change byte error. If this probability is
	 * &gt; 0 the filter will modify a random number of byte of the processed
	 * {@link IoBuffer}.
	 * 
	 * @param changeByteProbability
	 *            probability of modifying an IoBuffer out of 1000 processed
	 *            {@link IoBuffer}
	 */
	...
	/**
	 * Set the probability for the insert byte error. If this probability is
	 * &gt; 0 the filter will insert a random number of byte in the processed
	 * {@link IoBuffer}.
	 * 
	 * @param insertByteProbability
	 *            probability of inserting in IoBuffer out of 1000 processed
	 *            {@link IoBuffer}
	 */
	...
	/**
	 * Set the probability for the remove byte error. If this probability is
	 * &gt; 0 the filter will remove a random number of byte in the processed
	 * {@link IoBuffer}.
	 * 
	 * @param removeByteProbability
	 *            probability of modifying an {@link IoBuffer} out of 1000
	 *            processed IoBuffer
	 */
	 ...
	 * {@link IoBuffer}. The default value is 10.
	 * 
	 * @param maxInsertByte
	 *            maximum bytes inserted in a {@link IoBuffer}
	 */
