...
/**
 * An {@link IoFilter} implementation used to buffer outgoing
 * {@link WriteRequest} almost like what {@link BufferedOutputStream} does.
 * Using this filter allows to be less dependent from network latency. It is
 * also useful when a session is generating very small messages too frequently
 * and consequently generating unnecessary traffic overhead.
 * 
 * Please note that it should always be placed before the
 * {@link ProtocolCodecFilter} as it only handles {@link WriteRequest}'s
 * carrying {@link IoBuffer} objects.
 * 
...
public final class BufferedWriteFilter extends IoFilterAdapter {
	...
	private static final String TAG = BufferedWriteFilter.class.getName();
	 ...
	 * default instance of {@link ConcurrentHashMap}.
	 * 
	 * @param bufferSize
	 *            the new buffer size
	 */
	 ...
	 * <code>buffersMap</code> is null then a default instance of
	 * {@link ConcurrentHashMap} is created else the provided instance is used.
	 * 
	 * @param bufferSize
	 *            the new buffer size
	 * @param buffersMap
	 ...
	 * 
	 * @throws Exception
	 *             if <code>writeRequest.message</code> isn't an
	 ...
	 * 
	 * @param session
	 *            the session to which a write is requested
	 * @param data
	 *            the data to buffer
	 */
	 ...
	 * <code>session</code> {@link IoSession} until buffer is full or manually
	 * flushed.
	 * 
	 * @param session
	 ...
	 *            the data to buffer
	 * @param buf
	 *            the buffer where data will be temporarily written
	 */
	 ...
	 * 
	 * @param nextFilter
	 *            the {@link NextFilter} of this filter
	 * @param session
	 *            the session where buffer will be written
	 * @param buf
	 *            the data to write
	...
	private void internalFlush(NextFilter nextFilter, IoSession session, IoBuffer buf) throws Exception {
		...
		Mina4Log.d(TAG, String.format("Flushing buffer: %s", tmp));
