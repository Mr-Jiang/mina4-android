...
public class SslFilter extends IoFilterAdapter {
	...
	private static final String TAG = SslFilter.class.getName();

	/**
	 * A session attribute key that stores underlying {@link SSLSession} for
	 * each session.
	 */
	public static final AttributeKey SSL_SESSION = new AttributeKey(SslFilter.class, "session");

	/**
	 * A session attribute key that makes next one write request bypass this
	 * filter (not encrypting the data). This is a marker attribute, which means
	 * that you can put whatever as its value. ({@link Boolean#TRUE} is
	 * preferred.) The attribute is automatically removed from the session
	 * attribute map as soon as {@link IoSession#write(Object)} is invoked, and
	 * therefore should be put again if you want to make more messages bypass
	 * this filter. This is especially useful when you implement StartTLS.
	 */
	 ...
	 * {@link IoHandler#messageReceived(IoSession, Object)} event with a special
	 * message ({@link #SESSION_SECURED} or {@link #SESSION_UNSECURED}). This is
	 * a marker attribute, which means that you can put whatever as its value. (
	 * {@link Boolean#TRUE} is preferred.) By default, this filter doesn't emit
	 * any events related with SSL session flow control.
	 */
	 ...
	 * {@link InetSocketAddress}. Setting this attribute causes
	 ...
	 * {@link SSLEngine} instance. If not set
	 * {@link SSLContext#createSSLEngine()} will be called. <br>
	 ...
	 * {@link IoHandler#messageReceived(IoSession, Object)} event when the
	 * session is secured and its {@link #USE_NOTIFICATION} attribute is set.
	 */
	 ...
	 * A special message object which is emitted with a
	 * {@link IoHandler#messageReceived(IoSession, Object)} event when the
	 * session is not secure anymore and its {@link #USE_NOTIFICATION} attribute
	 * is set.
	 */
	...
	/**
	 * Creates a new SSL filter using the specified {@link SSLContext}. The
	...
	/**
	 * Creates a new SSL filter using the specified {@link SSLContext}. If the
	 * <tt>autostart</tt> flag is set to <tt>true</tt>, the handshake will start
	 * immediately.
	 * 
	 * @param sslContext
	 *            The SSLContext to use
	 * @param autoStart
	 *            The flag used to tell the filter to start the handshake
	 *            immediately
	 */
	 ...
	 * yet. Please note that SSL session is automatically started by default,
	 * and therefore you don't need to call this method unless you've used TLS
	 * closure.
	 *
	 * @param session
	 *            The session that will be switched to SSL mode
	...
	public boolean startSsl(IoSession session) throws SSLException {
		...
		return started;
			 ...
			 * is not yet completed, we will print (ssl) in small caps. Once
			 * it's completed, we will use SSL capitalized.
			 */
	 ...
	 *         encrypted/decrypted over SSL/TLS currently. This method will
	 *         start to return <tt>false</tt> after TLS <tt>close_notify</tt>
	 *         message is sent and any messages written after then is not going
	 *         to get encrypted.
	 * 
	 * @param session
	 *            the session we want to check
	 */
	...
	/**
	 * @return <tt>true</tt> if the engine is set to use client mode when
	 *         handshaking.
	 */
	 ...
	 * 
	 * @param clientMode
	 *            <tt>true</tt> when we are in client mode, <tt>false</tt> when
	 *            in server mode
	 */
	...
	/**
	 * Configures the engine to <em>require</em> client authentication. This
	...
	/**
	 * Configures the engine to <em>request</em> client authentication. This
	 * option is only useful for engines in the server mode.
	 * 
	...
	public void onPreAdd(IoFilterChain parent, String name, NextFilter nextFilter) throws SSLException {
		...
		Mina4Log.e(TAG, "Adding the SSL Filter " + name + " to the chain");
	...
	public void messageReceived(NextFilter nextFilter, IoSession session, Object message) throws SSLException {
		Mina4Log.d(TAG, getSessionInfo(session) + ": Message received : " + message);
		...
		synchronized (sslHandler) {
			if (!isSslStarted(session) && sslHandler.isInboundDone()) {
				...
				try {
					...
					// Handle data to be forwarded to application or written to
					// net
					...
					if (!sslHandler.isHandshakeComplete()) {
						...
						// Close the session immediately, the handshake has
						// failed
						session.closeNow();
					} else {
	...
	public void filterWrite(NextFilter nextFilter, IoSession session, WriteRequest writeRequest) throws SSLException {
		Mina4Log.d(TAG, getSessionInfo(session) + ": Writing Message : " + writeRequest);
				...
				// Don't encrypt the data if encryption is disabled.
					...
					if (sslHandler.isWritingEncryptedData()) {
						...
						sslHandler.scheduleFilterWrite(nextFilter,
								new EncryptedWriteRequest(writeRequest, encryptedBuffer));
					} else {
	...
	/**
	 * Initiate the SSL handshake. This can be invoked if you have set the
	 * 'autoStart' to false when creating the SslFilter instance.
	 * 
	 * @param session
	 *            The session for which the SSL handshake should be done
	 * @throws SSLException
	...
	private void initiateHandshake(NextFilter nextFilter, IoSession session) throws SSLException {
		Mina4Log.d(TAG, getSessionInfo(session) + " : Starting the first handshake");
		SslHandler sslHandler = getSslSessionHandler(session);
	...
	private void handleSslData(NextFilter nextFilter, SslHandler sslHandler) throws SSLException {
		Mina4Log.d(TAG, getSessionInfo(sslHandler.getSession()) + ": Processing the SSL Data");
	 ...
	 *
	 * @author <a href="https://github.com/Mr-Jiang/mina4-android">mina4-android
	 *         </a>
	 */
