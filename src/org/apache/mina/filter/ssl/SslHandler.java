...
class SslHandler {
	...
	private static final String TAG = SslHandler.class.getName();
	...
	/**
	 * A flag set to true when the first SSL handshake has been completed This
	 * is used to avoid sending a notification to the application handler when
	 * we switch to a SECURE or UNSECURE session.
	 */
	...
	/**
	 * A flag used to indicate to the SslFilter that the buffer it will write is
	 * already encrypted (this will be the case for data being produced during
	 * the handshake).
	 */
	...
	/* no qualifier */ SslHandler(SslFilter sslFilter, IoSession session) throws SSLException {
		...
		this.session = session;
			 ...
			 *
			 * @throws SSLException
			 *             If the underlying SSLEngine handshake initialization
			 *             failed
			 */
		...
		Mina4Log.d(TAG, sslFilter.getSessionInfo(session) + " Initializing the SSL Handler");
		...
		Mina4Log.d(TAG, sslFilter.getSessionInfo(session) + " SSL Handler Initialization done.");
	}
	...
	@SuppressLint("TrulyRandom")
		...
		try {
			...
			Mina4Log.d(TAG, "Unexpected exception from SSLEngine.closeInbound().\n" + e.toString());
		}
		...
		while ((scheduledWrite = preHandshakeEventQueue.poll()) != null) {
			sslFilter.filterWrite(scheduledWrite.getNextFilter(), session,
		...
		filterWriteEventQueue.add(new IoFilterEvent(nextFilter, IoEventType.WRITE, session, writeRequest));
			...
			/**
			 * Push the newly received data into a queue, waiting for the SSL
			 * session to be fully established
			 *
			 * @param nextFilter
			 *            The next filter to call
			 * @param message
			 *            The incoming data
			 */
		...
		if (sslLock.tryLock()) {
			...
			try {
				do {
					// We need synchronization here inevitably because
					// filterWrite can be
					// called simultaneously and cause 'bad record MAC'
					// integrity error.
			...
			/**
			 * Call when data are read from net. It will perform the initial
			 * hanshake or decrypt the data if SSL has been initialiaed.
			 * 
			 * @param buf
			 *            buffer to decrypt
			 * @param nextFilter
		...
		if (!isOutboundDone()) {
			Mina4Log.d(TAG, sslFilter.getSessionInfo(session) + " Processing the received message");
		} else {
			Mina4Log.d(TAG, sslFilter.getSessionInfo(session) + " Processing the received message");
		}
		...
		return answer.shrink();
			...
			/**
			 * Encrypt provided buffer. Encrypted data returned by
			 * getOutNetBuffer().
			 * 
			 * @param src
			 *            data to encrypt
	...
	private void checkStatus(SSLEngineResult res) throws SSLException {
		...
		for (;;) {
			...
			case NOT_HANDSHAKING:
				Mina4Log.d(TAG, sslFilter.getSessionInfo(session) + " processing the FINISHED state");
				...
				if (!isOutboundDone()) {
					Mina4Log.d(TAG, sslFilter.getSessionInfo(session) + " is now secured");
				} else {
					Mina4Log.d(TAG, sslFilter.getSessionInfo(session) + " is not secured yet");
				}

				return;

			case NEED_TASK:
				Mina4Log.d(TAG, sslFilter.getSessionInfo(session) + " processing the NEED_TASK state");

				handshakeStatus = doTasks();
				break;

			case NEED_UNWRAP:
				Mina4Log.d(TAG, sslFilter.getSessionInfo(session) + " processing the NEED_UNWRAP state");
			...
			case NEED_WRAP:
				Mina4Log.d(TAG, sslFilter.getSessionInfo(session) + " processing the NEED_WRAP state");
		...
		try {
			...
			while (needToCompleteHandshake()) {
				...
				if (currentOutNetBuffer != null && currentOutNetBuffer.hasRemaining()) {
					writeFuture = new DefaultWriteFuture(session);
					sslFilter.filterWrite(nextFilter, session,
							new DefaultWriteRequest(currentOutNetBuffer, writeFuture));
				}
			}
		} finally {
			writingEncryptedData = false;
	...
	private SSLEngineResult.Status unwrapHandshake(NextFilter nextFilter) throws SSLException {
		...
		if ((handshakeStatus == SSLEngineResult.HandshakeStatus.FINISHED)
				&& (res.getStatus() == SSLEngineResult.Status.OK) && inNetBuffer.hasRemaining()) {
			res = unwrap();
