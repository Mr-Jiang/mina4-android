...
public final class NioSocketAcceptor extends AbstractPollingIoAcceptor<NioSession, ServerSocketChannel>
	...
	private static final String TAG = NioSocketAcceptor.class.getName();
	...
	/**
	 * Constructor for {@link NioSocketAcceptor} with default configuration but
	...
	/**
	 * Constructor for {@link NioSocketAcceptor} with a given {@link Executor}
	 ...
	 * Constructor for {@link NioSocketAcceptor} using default parameters, and
	 * given number of {@link NioProcessor} for multithreading I/O operations,
	 * and a custom SelectorProvider for NIO
	 *
	 * @param processorCount
	 *            the number of processor to create and place in a
	 * @param selectorProvider
	 *            teh SelectorProvider to use {@link SimpleIoProcessorPool}
	 */
	...
	protected NioSession accept(IoProcessor<NioSession> processor, ServerSocketChannel handle) throws Exception {
		...
		try {
			...
			Mina4Log.e(TAG, "Error Calling Accept on Socket - Sleeping Acceptor Thread. Check the ulimit parameter\n"
					+ t.toString());
			try {
				// Sleep 50 ms, so that the select does not spin like crazy
				// doing nothing but eating CPU
				// This is typically what will happen if we don't have any more
				// File handle on the server
				// Check the ulimit parameter
				// NOTE : this is a workaround, there is no way we can handle
				// this exception in any smarter way...
				Thread.sleep(50L);
	...
	private static class ServerSocketChannelIterator implements Iterator<ServerSocketChannel> {
		 ...
		 * 
		 * @return <tt>true</tt> if there is at least one more SockectChannel
		 *         object to read
		 */
